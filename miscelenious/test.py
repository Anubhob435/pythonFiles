...
def complete_program():
    ...
    if balance > 0:
        ...
        g = 2
        l = int(input("guess a random number between 1 and 3: "))
        if g == l:
            ...
            try:
                mydb = mysql.connector.connect(host="buh89x1pi8cgvaw4161i-mysql.services.clever-cloud.com",
                                               ...
                                               database="buh89x1pi8cgvaw4161i")
                cursor = mydb.cursor()
                k = (sum(values) - (0.5 * sum(values)))
                print (k)
                quary2 = """UPDATE abc SET balance = balance + %s WHERE phone = %s"""
                value2 = (k, ph_no)
                cursor.execute(quary2, value2)
                mydb.commit()
                sql_select_Query = """select * from abc WHERE phone = %s"""
                cursor.execute(sql_select_Query, (ph_no,))
                records = cursor.fetchall()
                print("Total number of rows in table: ", cursor.rowcount)
                for i in records:
                    ball = i[4]
                print("remaining balance is:", ball)
            except mysql.connector.Error as error:
                print("Failed to insert into MySQL table {}".format(error))
        else:
